...
When it closes the buffer, it immediately opens a new buffer and repeats the process.`,
	...
	window: {
		categories: ['subscription', 'transformation'],
		img: 'window.png',
		url:
...
It's like buffer, but emits a nested Observable instead of an array.",
	...
	windowCount: {
		...
		description:
			'Branch out the source Observable values as a nested Observable with each nested Observable \
emitting at most `windowSize` values.',
	},

	windowTime: {
		categories: ['subscription', 'transformation', 'filter'],
		img: 'windowTime.png',
...
in a serialized fashion, waiting for each projected observable to complete before merging the next.',
	...
	concatMapTo: {
		...
		description:
			'Projects each source value to the ***same*** Observable which is merged multiple times in \
a serialized fashion on the output Observable.',
	},

	exhaustMap: {
		categories: ['subscription', 'transformation', 'data', 'time', 'filter'],
		img: 'exhaustMap.png',
		url:
...
repeat this process.`,
	...
	mergeMap: {
		...
		description:
			'Projects each source value to an Observable which is merged with all projected observables, \
...
in the output Observable.',
	...
	expand: {
		...
		img: 'expand.png',
		url:
			'http://reactivex.io/rxjs/class/es6/Observable.js~Observable.html#instance-method-expand',
...
The output Observables emits values only from the most recently emitted instance of innerObservable.`,
	...
	groupBy: {
		...
		img: 'groupBy.png',
		url:
			'http://reactivex.io/rxjs/class/es6/Observable.js~Observable.html#instance-method-groupBy',
		description:
...
this operator applies a projection to each value and emits that projection in the output Observable.',
	...
	mapTo: {
		...
		description:
			'Emits the ***same*** given constant value on the output Observable \
...
but not on the first emission, because there is no previous value in that case.`,
	...
	mergeScan: {
		categories: ['transformation', 'subscription', 'data'],
		url:
...
It's like delay, but the time span of the delay duration is determined by a second Observable.`,
	...
	materialize: {
		categories: ['transformation', 'data', 'completion', 'error'],
		img: 'materialize.png',
		url:
			'http://reactivex.io/rxjs/class/es6/Observable.js~Observable.html#instance-method-materialize',
...
It unwraps \`Notification\` objects as actual *next*, *error* and *complete* emissions. The opposite of materialize.`,
	...
	subscribeOn: {
		categories: ['transformation', 'subscription'],
		img: 'subscribeOn.png',
		url:
			'http://reactivex.io/rxjs/class/es6/Observable.js~Observable.html#instance-method-subscribeOn',
		description:
			'Returns an observable that subscribes asynchronously to the source-observable \
on the provided *scheduler*.',
	...
	observeOn: {
		categories: ['transformation', 'subscription'],
		url:
			'http://reactivex.io/rxjs/class/es6/Observable.js~Observable.html#instance-method-observeOn',
		description:
