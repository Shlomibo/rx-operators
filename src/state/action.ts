...
import { Lens, Reducer } from 'cycle-onionify';
...
export type ActionDescriptor<T = undefined> = string | Action<T>;
...
export function reduce<State extends object, Payloads extends PayloadsType<State>>(
...
function isIn<ObjType extends object>(key: string, obj: ObjType): key is keyof ObjType {
	return key in obj;
}
...
export function select<State extends object, Key extends keyof State>(
	key: Key
): (source: Observable<State | undefined>) => Observable<State[Key] | undefined> {
	return source => source.map(state => state && state[key]).distinctUntilChanged();
}

export function BANG<T>(source: Observable<T | null | undefined>): Observable<T> {
